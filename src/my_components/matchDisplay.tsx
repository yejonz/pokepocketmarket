...
import { doc, getDoc, getFirestore, serverTimestamp, setDoc, type Timestamp } from "firebase/firestore"
            ...
            <AccordionTrigger onClick={preventToggle} className="cursor-default">
              <Card className="flex">
                <div className="flex flex-col">
                  <h3 className="text-xl font-mono font-semibold mb-4 mt-4 ml-4">Select a card you have:</h3>
                  <div
                    style={{
                      display: "flex",
                      flexWrap: "wrap",
                      width: `${cardWidth * cardsPerRow + cardsPerRow}${cardWidthMode}`,
                      padding: "1rem",
                  ...
                  >
                    {data.userHasOtherWants.map((card) => (
                      <div
                        key={card}
                        style={{ width: `${cardWidth}${cardWidthMode}`, maxWidth: "294px", margin: ".25rem" }}
                      >
                        <div onClick={() => selectHaveCard(card)}>
                          <div
                            className={
                              card === selectHC
                                ? "ring-4 ring-primary shadow-lg scale-105 transition-all z-10 relative"
                ...
                <div className="flex flex-col">
                  <h3 className="text-xl font-mono font-semibold mb-4 mt-4 ml-4">Select a card you want:</h3>
                  <div
                    style={{
                      display: "flex",
                      flexWrap: "wrap",
          ...
          <AccordionContent style={{ width: triggerWidth }}>
            <Card className="p-4 flex">
              ...
              <ol className="w-1/2 ml-4">
                <li className="mb-2">
                  <b>Last Active: </b>
                  {timeAgo(data.lastActive.toDate())}
                </li>
                ...
                <li className="flex gap-1 mb-2 mt-2">
                  <b>Discord:</b>
                  {data.discord ? data.discord : <p className="text-gray-400 italic">blank</p>}
                </li>
                <li className={sendTradeMsg ? "mt-2" : "mb-9 mt-2"}>
                  ...
                  {sendTradeMsg && (
                    <div
                      className={
                        sendTradeMsg == "Trade sent successfully."
                          ? "mt-2 mb-2 italic text-green-600"
                          : "mt-2 mb-2 italic text-red-600"
                      }
                    >
                      {sendTradeMsg}
                    </div>
