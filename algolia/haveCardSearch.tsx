    ...
    <ol style={{ display: 'flex', flexWrap: 'wrap', width: `${cardWidth * cardsPerRow}${cardWidthMode}`}}>
      {items.map((hit) => (
        <li
          ...
          onClick={() => {
            if (!cardState?.addHC.includes(hit.objectID) && !cardState?.haveArr.includes(hit.objectID)) {
              cardState?.setAddHC((prev) => [...prev, hit.objectID]);
            }
          }}
        >
...
export default function HaveCardSearch() {
  return (
    <div>
